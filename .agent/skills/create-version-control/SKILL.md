...
5. **commit message 格式**
   - commit message 必須先使用 `[]` 標示本次提交的功能或目的。
   - `[]` 後方必須以繁體中文簡短列出改動項目。
   - 改動項目需聚焦實際變更，避免冗長描述或英文慣用前綴。
   - 格式範例：`[版本控管] 更新 commit 與 push 確認流程`。
   - 若變更範圍較大，可使用分號或頓號分隔重點，例如：`[簽核申請] 新增申請入口、我的申請與簽核檢視`。
